...
class Transform:
    def __init__(self,):
        ...
        self._source_rgba: Optional[np.ndarray] = None
        ...
        self.frame_rgba: Optional[np.ndarray] = None
    ...
    def set_frame_shape(self, frame_shape: tuples.ImageShape):
        ...
        self.frame_rgba = np.zeros(shape=(_y_pixels, _x_pixels, 4), dtype=np.uint8)
        self.updated = False

    def set_mandel(self, mandel: mandelbrot.Mandel):
        self._offset = mandel.offset
        self._iteration = mandel.iteration
        mod_log_iterations = np.mod(np.log10(1 + self._iteration), 1)
        ...
        self._source_rgba = self._cmap(mod_log_iterations, bytes=True)
    ...
    def pan(self, pan: tuples.PixelPoint) -> np.ndarray:
        self._transform_vector = np.array([-self._offset.x + pan.x,
